<script lang="ts">
  ...
  const insightText = $derived.by(() => {
    ...
    if (slope > 0.02) {
      return `Positive correlation (slope: +${slope}%/s, R²: ${r2}). Taking more time is associated with higher accuracy—rushing leads to mistakes.`;
    } else if (slope < -0.02) {
      return `Negative correlation (slope: ${slope}%/s, R²: ${r2}). You tend to be more accurate when solving fluidly; prolonged hesitations correspond to harder or failed tests.`;
    } else {
      return `Neutral correlation (slope: ${slope}%/s, R²: ${r2}). Accuracy remains relatively steady regardless of test duration.`;
    }
  });
...
<div class="bg-[#FAF0DA] border border-[#D6BA96] rounded-xl p-4 shadow-xs">
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
    ...
    <div class="inline-flex items-center gap-2 bg-[#FDF5E6] border border-[#D6BA96] px-2.5 py-1 rounded-lg text-xs self-start sm:self-auto">
      ...
      <span class="text-[#5e4537]">R²: <strong class="text-[#3D2A1F]">{regression.r2}</strong></span>
    </div>
  </div>
